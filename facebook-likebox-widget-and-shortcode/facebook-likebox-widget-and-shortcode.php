...
class AWPLIFW_FB_Widget_Shotcode extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			...
			__( 'Facebook Likebox Widget', FLB_TXTDM ), // Name
	...
	public function widget( $args, $instance ) {
		...
		$app_id 		= isset($instance['app_id'])		? esc_attr($instance['app_id']) : "1869036243369971";
		$fb_page_url 	= isset($instance['fb_page_url']) 	? esc_url($instance['fb_page_url']) : "https://www.facebook.com/AWordPressLife/";
		$title 			= isset($instance['title']) 		? esc_attr($instance['title']) : "false";
		$w_auto_width 	= isset($instance['w_auto_width']) 	? esc_attr($instance['w_auto_width']) : "false";
		$w_width 		= isset($instance['w_width']) 		? esc_attr($instance['w_width']) : "";
		$w_height 		= isset($instance['w_height']) 		? esc_attr($instance['w_height']) : "";
		$cover_photo 	= isset($instance['cover_photo']) 	? esc_attr($instance['cover_photo']) : "true";
		$header_size 	= isset($instance['header_size']) 	? esc_attr($instance['header_size']) : "true";
		$show_fans 		= isset($instance['show_fans']) 	? esc_attr($instance['show_fans']) : "true";
		$show_post 		= isset($instance['show_post']) 	? esc_attr($instance['show_post']) : "true";
		$language 		= isset($instance['language']) 		? esc_attr($instance['language']) : "en_US";
		$credit_link 	= isset($instance['credit_link']) 	? esc_attr($instance['credit_link']) : "false";

		echo $args['before_title'] . apply_filters('widget_title', esc_html($instance['w_title'])) . $args['after_title'];
	...
	public function form( $instance ) {
		...
		$w_title 		= !empty($instance['w_title']) 			? esc_attr($instance['w_title']) : '';
		$app_id 		= !empty($instance['app_id']) 			? esc_attr($instance['app_id']) : '1869036243369971';
		$fb_page_url 	= !empty($instance['fb_page_url']) 		? esc_url($instance['fb_page_url']) : 'https://www.facebook.com/AWordPressLife/';
		$w_width 		= !empty($instance['w_width']) 			? esc_attr($instance['w_width']) : '250';
		$w_height 		= !empty($instance['w_height']) 		? esc_attr($instance['w_height']) : '';
		$w_auto_width 	= !empty($instance['w_auto_width']) 	? esc_attr($instance['w_auto_width']) : 'false';
		$cover_photo 	= !empty($instance['cover_photo']) 		? esc_attr($instance['cover_photo']) : 'false';
		$header_size 	= !empty($instance['header_size']) 		? esc_attr($instance['header_size']) : 'false';
		$show_post 		= !empty($instance['show_post']) 		? esc_attr($instance['show_post']) : 'true';
		$show_fans 		= !empty($instance['show_fans']) 		? esc_attr($instance['show_fans']) : 'true';
		$language 		= !empty($instance['language']) 		? esc_attr($instance['language']) : 'en_US';
		$credit_link 	= !empty($instance['credit_link']) 		? esc_attr($instance['credit_link']) : 'false';
		...
		<p>
		<label for="<?php echo $this->get_field_id( 'app_id' , FLB_TXTDM ); ?>"><?php _e( 'Facebook Application ID' , FLB_TXTDM ); ?></label> &nbsp;  (<a href="https://awplife.com/create-facebook-application-id/" target="_new">Create New Own Application ID</a>)
		<input class="widefat" id="<?php echo $this->get_field_id( 'app_id' , FLB_TXTDM ); ?>" name="<?php echo $this->get_field_name( 'app_id' , FLB_TXTDM ); ?>" type="number" value="<?php echo esc_attr( $app_id , FLB_TXTDM ); ?>" placeholder="Enter Facebook Application ID Here">
		</p>
		...
		<p>
		<label for="<?php echo $this->get_field_id( 'w_width' , FLB_TXTDM ); ?>"><?php _e( 'Custom Widget Width' , FLB_TXTDM ); ?> (<?php _e( 'Min Width: 180 - Max Width: 500' , FLB_TXTDM ); ?>)</label> 
		<input class="widefat" id="<?php echo $this->get_field_id( 'w_width' , FLB_TXTDM ); ?>" name="<?php echo $this->get_field_name( 'w_width' , FLB_TXTDM ); ?>" type="number" value="<?php echo esc_attr( $w_width , FLB_TXTDM ); ?>" placeholder="(Min Width: 180 - Max Width: 500)">
		</p>
		
		<p>
		<label for="<?php echo $this->get_field_id( 'w_height' , FLB_TXTDM ); ?>"><?php _e( 'Custom Widget Height' , FLB_TXTDM ); ?> (<?php _e( 'Minimium Height: 70' , FLB_TXTDM ); ?>)</label> 
		<input class="widefat" id="<?php echo $this->get_field_id( 'w_height' , FLB_TXTDM ); ?>" name="<?php echo $this->get_field_name( 'w_height' , FLB_TXTDM ); ?>" type="number" value="<?php echo esc_attr( $w_height , FLB_TXTDM ); ?>" placeholder="(Minimium Height: 70)">
		</p>
		...
		<p>
		<label for="<?php echo $this->get_field_id( 'credit_link' , FLB_TXTDM ); ?>"><?php _e( 'Show Plugin Credit Link' , FLB_TXTDM ); ?></label><br>
		<input class="widefat" id="<?php echo $this->get_field_id( 'credit_link' , FLB_TXTDM ); ?>" name="<?php echo $this->get_field_name( 'credit_link' , FLB_TXTDM ); ?>" <?php if($credit_link == 'true') echo "checked=checked" ?> type="radio" value="true"> Yes &nbsp;&nbsp;
		<input class="widefat" id="<?php echo $this->get_field_id( 'credit_link' , FLB_TXTDM ); ?>" name="<?php echo $this->get_field_name( 'credit_link' ); ?>" <?php if($credit_link == 'false') echo "checked=checked" ?> type="radio" value="false"> No
		</p>
		<?php 
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['w_title'] 	= !empty($new_instance['w_title']) 		? sanitize_text_field($new_instance['w_title']) : '';
		$instance['app_id'] 	= !empty($new_instance['app_id']) 		? sanitize_text_field($new_instance['app_id']) : '1869036243369971';
		$instance['w_width'] 	= !empty($new_instance['w_width']) 		? sanitize_text_field($new_instance['w_width']) : '250';
		$instance['w_height'] 	= !empty($new_instance['w_height']) 	? sanitize_text_field($new_instance['w_height']) : '';
		$instance['language'] 	= !empty($new_instance['language']) 	? sanitize_text_field($new_instance['language']) : 'en_US';

		// Sanitize URL fields
		$instance['fb_page_url'] = !empty($new_instance['fb_page_url']) 	? esc_url_raw($new_instance['fb_page_url']) : 'https://www.facebook.com/A-WordPress-Life-1020764421361036/';

		// Sanitize boolean fields as text (since they are likely stored as 'true'/'false' strings, consider checking for boolean values if storing as actual booleans)
		$instance['w_auto_width'] 	= !empty($new_instance['w_auto_width']) ? sanitize_text_field($new_instance['w_auto_width']) : 'false';
		$instance['cover_photo'] 	= !empty($new_instance['cover_photo']) 	? sanitize_text_field($new_instance['cover_photo']) : 'false';
		$instance['header_size'] 	= !empty($new_instance['header_size']) 	? sanitize_text_field($new_instance['header_size']) : 'false';
		$instance['show_fans'] 		= !empty($new_instance['show_fans']) 	? sanitize_text_field($new_instance['show_fans']) : 'true';
		$instance['show_post'] 		= !empty($new_instance['show_post']) 	? sanitize_text_field($new_instance['show_post']) : 'true';
		$instance['credit_link'] 	= !empty($new_instance['credit_link']) 	? sanitize_text_field($new_instance['credit_link']) : 'false';
	...
	//Plugin Recommend
		add_action('tgmpa_register','FLB_TXTDM_plugin_recommend');
		function FLB_TXTDM_plugin_recommend(){
			$plugins = array(
				array(
