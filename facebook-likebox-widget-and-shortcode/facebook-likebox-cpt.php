...
if ( ! class_exists( 'Fb_Likebox_Shortcode_CPT' ) ) {
	...
	class Fb_Likebox_Shortcode_CPT {
		...
		protected function _constants() {
			//Plugin Version
			define( 'FLWS_PLUGIN_VER', '1.2.1' );
			
			//Plugin Text Domain
			define("FLB_TXTDM","facebook-likebox-widget-and-shortcode" );

			//Plugin Name
			define( 'FLWS_PLUGIN_NAME', __( 'Profile Box Shortcode And Widget', FLB_TXTDM ) );
		...
		public function _fb_save_settings($post_id) {
			if ( isset( $_POST['fb-settings'] ) == "fb-save-settings" ) {
				...
				$fb_shortcode_settings_array = array (
					'app_id' => sanitize_text_field($_POST['app_id']),
					'fb_page_url' => sanitize_text_field($_POST['fb_page_url']),
					'title' => sanitize_text_field($_POST['title']),
					'w_auto_width' => sanitize_text_field($_POST['w_auto_width']),
					'w_width' => sanitize_text_field($_POST['w_width']),
					'w_height' => sanitize_text_field($_POST['w_height']),
					'cover_photo' => sanitize_text_field($_POST['cover_photo']),
					'header_size' => sanitize_text_field($_POST['header_size']),
					'show_fans' => sanitize_text_field($_POST['show_fans']),
					'show_post' => sanitize_text_field($_POST['show_post']),
					'language' => sanitize_text_field($_POST['language']),
					'credit_link' => sanitize_text_field($_POST['credit_link']),
				);
				
				update_post_meta($post_id, $awl_fb_shortcode_setting, $fb_shortcode_settings_array);
				//echo 2;
				//die;
			}
		}// end save setting
